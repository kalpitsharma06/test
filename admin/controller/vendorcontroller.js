...
exports.addrestaurant = async function (req, res, next) {
  try {
    ...
    let payload = {
      cordinates: [longitude, lattitude],
    };
...
exports.Accept_status_restro = async (req, res) => {
  try {
    const ResDetails = await signUp.findByIdAndUpdate(
      req.params.id,
      {
        status: true,
      },
    ...
    res.status(200).json({
      status: true,
      message: 'Successfully Accepted Resturent status ',
      results: ResDetails,
    });
  } catch (error) {
    res.status(400).json(error.message);
...
exports.Reject_status_restro = async (req, res) => {
  try {
    signUp.findByIdAndUpdate(req.params.id, { status: false }, { new: true }, (err, data) => {
      if (err) {
        res.status(400).json(error.message);
      } else {
        console.log(data);
        res.status(200).json({
          status: true,
          message: 'Successfully Rejected Resturent status ',
          results: data,
        });
      }
    });
