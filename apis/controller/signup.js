...
exports.updateResturantDetails = async (req, res) => {
  try {
    let sssssss = await signUp.findByIdAndUpdate(
      req.params.id,
      {
        restaurant_name: req.body.restaurant_name,
        owner_name: req.body.owner_name,
...
exports.vendor_report_betweendates = async (req, res) => {
  ...
  orderModel.find({ vendorID: id }, (err, order_data) => {
    ...
    if (err) throw err;
    const orders = order_data.filter(
      (x) =>
        moment(x.createdAt).format('YYYY-MM-DD') >= startDate && moment(x.createdAt).format('YYYY-MM-DD') <= endDate
    );
