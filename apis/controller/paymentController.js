...
exports.Default_card = async (req, res, next) => {
  ...
  User.find({ _id: ObjectId(logindata.id) }, (err, usedata) => {
    ...
    stripe
      .updateCustomerCard(stripe_id, card_id)
      .then(function () {
        User.update(
          {
            _id: logindata.id,
    ...
    if (stripe_id == undefined || stripe_id == '') {
      stripe
        ...
        .then(function (customer) {
          ...
          stripe
            .createCardToken(reqdata)
            .then(function (token) {
              console.log('sadasdasd', customer, token);
              stripe
                .addSource(customer.id, token.id)
                .then(function (source) {
                  ...
                  User.update(
                    ...
                    function (err) {
                      if (err) {
                        stripe
                          .deleteSource(source.customer, source.id)
                          .then(function () {
                            return next(err);
                          })
                          .catch(function (err) {
